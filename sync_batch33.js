...
const ids = [1876, 1877, 1878, 1879, 1880];

async function syncBatch33() {
  const connection = await mysql.createConnection(dbConfig);
  let count = 0;
  for (const id of ids) {
    const filePath = path.join(__dirname, `temp/generated_products/${id}.json`);
    if (!fs.existsSync(filePath)) { console.log(`⚠️  Skipping ${id} - file not found`); continue; }
  ...
  console.log(`\n🎉 Batch 33 sync complete! ${count} products updated in MySQL database.`);
}

syncBatch33().catch(err => { console.error('❌ Error:', err); process.exit(1); });
